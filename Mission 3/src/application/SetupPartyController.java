...
public class SetupPartyController extends PollTrackerController {
    ...
    private int partyToChangeIndex = -1;
    ...
    void SetPartyButtonClicked(ActionEvent event) {
    	if(PartyNameTextField.getText().equals("")) {
    		errorMessage.setText("Are yo sure the party name is empty?");
    		System.out.println("The party name is empty.");
    	}
    	try {
    		value[partyToChangeIndex] = PartyNameTextField.getText();
        	SelectPartyChoiceBox.setItems(FXCollections.observableArrayList(value));
    	}catch(ArrayIndexOutOfBoundsException e) {
    		errorMessage.setText("Please select the party you want to set up.");
    		System.out.println("Please select the party you want to set up first.");
    	}
    	
    	//Factory aNewFactory = getFactory();
    	//Party[] theNewPartyList = null;
    	//for(int i=0; i<aNewFactory.getPartyNames().length; i++) {
    		//theNewPartyList[i] = aNewFactory.createRandomParty(value[i], 1000, 1);
    //	}
    ...
    void SubmitButtonClicked(ActionEvent event) {
    	getFactory().setPartyNames(value);
	...
	public void refresh() {
		PartyNameTextField.clear();
		value = getFactory().getPartyNames().clone();
    	SelectPartyChoiceBox.setItems(FXCollections.observableArrayList(value));
