...
public class AddPollViewController extends PollTrackerController{
    ...
    void AddThePoll(ActionEvent event) throws InvalidPartyDataException {
    	...
    	//This if statement used to handle there is no poll name entered.
    	if(userInputFromTextField.equals("")) {
    		errorMessage.setText("Are you sure the poll name you want to add is empty?");
    		System.out.println("The poll name you want to add is empty.");
    	}
    	...
    	//This try-catch block is used to handle the event where no poll was selected. 
    	try{
    		listOfPolls[indexOfPollChange] = newPoll;
    	}catch(ArrayIndexOutOfBoundsException e) {
    		errorMessage.setText("Please select a poll to replace.");
    		System.out.println("You should select a poll.");
    	}
